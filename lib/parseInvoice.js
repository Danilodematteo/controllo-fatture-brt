...
function parseInvoiceText(text) {
  ...
  // Alcune righe (spedizioni "reso"/giacenza, spesso col codice cliente 280 in testa)
  // NON hanno le colonne peso tassabile e trasporto: dopo il peso reale c'è subito
  // un codice varie (es. "I" = SPESE GIACENZA). Senza questa regex, il parser le
  // scambiava per continuazione della spedizione precedente e ne sommava i costi
  // a quelli — un cliente completamente diverso.
  const rowRe2 = /(\d{6,13})\s+(\d{3,4})\s+(?:(\d+)\s+)?([A-Z]{2})\s+([A-Z]{2})\s+([A-Z][A-Z'\.\s]*?)\s+(\d{5})\s+(\d{1,3})\s+([\d,\.]+)\s+([\d,\.]+)\s+([a-zA-Z])(?=\s)/;
  ...
  lines.forEach((line) => {
    ...
    if (m) {
      closeCurrent(current);
      const [, sped, dataSpedRaw, riferimento, provinciaArrivo, provinciaPartenza, nominativo, cap, colli, , pesoReale, pesoTass, trasporto, codiceTrasporto] = m;
    ...
    const m2 = line.match(rowRe2);
    if (m2) {
      closeCurrent(current);
      const [, sped, dataSpedRaw, riferimento, provinciaArrivo, provinciaPartenza, nominativo, cap, colli, , pesoReale, codiceExtra] = m2;
      const dataSpedizione = dataSpedRaw && dataSpedRaw.length === 4
        ? `${dataSpedRaw.slice(0, 2)}/${dataSpedRaw.slice(2, 4)}`
        : "";
      current = {
        sped, dataSpedizione, riferimento: riferimento || "", provinciaPartenza, provinciaArrivo, nominativo, cap,
        colli: parseInt(colli, 10) || 1,
        codiceTrasporto: codiceExtra,
        pesoReale: parseFloat(pesoReale.replace(",", ".")),
        pesoTass: parseFloat(pesoReale.replace(",", ".")), // non indicato su questo formato di riga: usiamo il peso reale
        trasporto: 0, // questo formato di riga (reso/giacenza) non ha una tariffa di trasporto separata
        senzaTrasporto: true,
        anchorLine: line,
        varieLines: [line.slice(m2.index + m2[0].length - 1)], // dal codice in poi
        allLines: [line],
      };
      return;
    }
